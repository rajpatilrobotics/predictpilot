...
describe('buildMintBinaryTx', () => {
  it('builds MarketKey first, then predict mint with DUSDC and clock', () => {
    const result = buildMintBinaryTx({ managerId, marketKey, quantityQuote, sender });
    ...
    expect(mintTarget).toBe(predictTxTargets.predict.mint);
  });
  ...
  it('fails safely when no sender is connected', () => {
    const result = buildMintBinaryTx({ managerId, marketKey, quantityQuote });
  ...
  it('fails safely for invalid object IDs and market key fields', () => {
    ...
    expect(invalidStrike).toMatchObject({
      error: {
        code: 'INVALID_INPUT',
        context: {
          field: 'strike1e9',
  ...
  it('fails safely when quantity is zero or negative', () => {
    const zeroResult = buildMintBinaryTx({ managerId, marketKey, quantityQuote: 0n, sender });
